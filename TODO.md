...
- [ ] **nutrient-deficiency-visual-diagnostic** (`docs/sims/nutrient-deficiency-visual-diagnostic/`)
    - `nutrient-deficiency-visual-diagnostic.js` uses the old multi-control-point `bezierVertex(...)` call — v2 takes one control point per `bezierVertex()` call — chain multiple calls instead of packing several points into one; use `bezierOrder()` for a quadratic curve.
